...
- **Data Access Layer**:
  ...
  - `MongoService.java`: Provides a direct connection to MongoDB (used for legacy or internal syncing).
...
All persistence operations are handled via the MongoDB Java Driver in `MongoService.java`:
- **Create**: `collection.insertOne(doc)` inside `addTask(Task task)`.
- **Read**: `collection.find(query)` inside `getTasks(String userId, String teamId)`.
- **Update**: `collection.updateOne(eq("_id", ...), combine(...))` inside `updateTask(Task t)`.
- **Delete**: `collection.deleteOne(eq("_id", ...))` inside `deleteTask(String id)`.
